...
const ShootingStars: React.FC = () => {
    ...
    useEffect(() => {
        // Visibility API: Pause star creation when the tab is not active
        const handleVisibilityChange = () => {
            if (document.hidden && intervalRef.current) {
                clearInterval(intervalRef.current);
                intervalRef.current = null;
            } else if (!document.hidden && !intervalRef.current) {
                intervalRef.current = setInterval(createStar, Math.random() * 2000 + 2000);
            }
        };

        document.addEventListener("visibilitychange", handleVisibilityChange);

        // Start creating stars when the component mounts
        intervalRef.current = setInterval(createStar, Math.random() * 2000 + 2000);

        return () => {
            // Cleanup: Stop creating stars and clear visibility event listener
            if (intervalRef.current) {
                clearInterval(intervalRef.current);
            }
            document.removeEventListener("visibilitychange", handleVisibilityChange);
            setStars([]); // Clear stars on unmount
        };
    }, []);
